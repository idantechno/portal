...
export default function ContactDetail() {
  ...
  // Seed the editable fields once the card loads (during render, not an effect,
  // so a fresh contact's data replaces stale edits rather than racing them).
  const [loadedContactId, setLoadedContactId] = useState<string | null>(null);
  if (card.data && loadedContactId !== card.data.contact.id) {
    setLoadedContactId(card.data.contact.id);
